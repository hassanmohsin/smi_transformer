...
def parse_arguments():
    ...
    parser.add_argument(
        "--out_dir", "-o", type=str, default="output", help="output directory"
    )
...
def main():
    ...
    for e in range(1, args.n_epoch):
        for b, (sm, prop) in tqdm(enumerate(train_loader)):
            ...
            if b % 50 == 0:
                print(
                    "Train {:3d}: iter {:5d} | pred. loss {:.3f} | rep. loss {:.3f} | total loss {:.3f} | ppl {:.3f}".format(
                        e,
                        b,
                        pred_loss.item(),
        ...
        eval_rep_loss, eval_pred_loss, eval_total_loss = evaluate(
            model, test_loader, vocab
        )
        print(
            "Val {:3d}: iter {:5d} | pred. loss {:.3f} | rep. loss {:.3f} | total loss {:.3f} | ppl {:.3f}".format(
                e,
                b,
                eval_pred_loss,
                eval_rep_loss,
        ...
        if not best_loss or eval_total_loss < best_loss:
            ...
            torch.save(
                ...
                "./%s/%s_%d_%d-%f.pkl"
                % (args.out_dir, args.name, e, b, eval_total_loss),
            )
